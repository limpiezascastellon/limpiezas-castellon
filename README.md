...
Esta web tiene **un único panel de administración** en **`/admin`** que te permite gestionarlo todo sin tocar código. Dentro tiene un menú lateral con **ocho secciones**:
...
6. **Horas registradas** → las horas que apuntan las empleadas en cada cliente, con precio/hora, importes, fines de semana marcados y exportación a Excel.
7. **Empleadas** → crear sus accesos y asignarles clientes.
8. **Clientes** → crear los clientes con su precio/hora.

Las empleadas tienen su propia página en **`/horas`**, donde apuntan desde el móvil las horas que hacen en cada cliente. Además pueden apuntarlas por WhatsApp con botones. Todo el control horario está explicado en **[HORAS.md](HORAS.md)**, y la puesta en marcha del bot en **[WHATSAPP.md](WHATSAPP.md)**.
