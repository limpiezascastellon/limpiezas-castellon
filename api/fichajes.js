...
//  últimos 2 meses. Los más antiguos desaparecen de su panel pero NUNCA se
...
const kv = (BD_URL && BD_TOKEN) ? createClient({ url: BD_URL, token: BD_TOKEN }) : null;
...
const MESES_VISIBLES_EMPLEADA = 2;      // ventana de la empleada
const DIAS_SESION = 30;                 // la empleada no tiene que entrar cada día

/* ===========================================================================
   UTILIDADES DE FECHA Y HORA (siempre en hora de España)
   =========================================================================== */
...
function esFecha(f) { return typeof f === "string" && /^\d{4}-\d{2}-\d{2}$/.test(f); }
function esHora(h) { return typeof h === "string" && /^([01]\d|2[0-3]):[0-5]\d$/.test(h); }

// Minutos trabajados. Si la salida es anterior a la entrada, se entiende que
// el turno cruza la medianoche (por ejemplo 22:00 → 02:00).
...
function mesDe(fecha) { return fecha.slice(0, 7); }          // "2026-09"
function sumaMeses(fecha, n) {
  const d = new Date(fecha + "T12:00:00Z");
  d.setUTCMonth(d.getUTCMonth() + n);
  return d.toISOString().slice(0, 10);
}
// Fecha más antigua que la empleada puede ver
function limiteEmpleada() { return sumaMeses(ahoraMadrid().fecha, -MESES_VISIBLES_EMPLEADA); }
...
async function mgetSeguro(claves) {
...
function comprobarPass(pass, emp) {
...
async function empleadaDeToken(req) {
  ...
  // Renovamos la sesión para que no caduque mientras se usa
...
// Devuelve los registros de un rango de fechas, con filtros opcionales
async function registrosEntre(desde, hasta, filtro = {}) {
...
// Añade a cada registro los datos calculados que necesita la pantalla
function decorar(r) {
...
async function borrarRegistro(reg) {
...
async function zonaEmpleada(accion, body, emp, res) {
  ...
  /* --- Mis horas de los últimos 2 meses --- */
  if (accion === "mios") {
  ...
  if (accion === "emp-crear") {
    ...
    const entrantes = Array.isArray(body.lineas) && body.lineas.length
      ? body.lineas
      : [{ cliId: body.cliId, fecha: body.fecha, entrada: body.entrada, salida: body.salida, notas: body.notas }];

    if (entrantes.length > 60) return res.status(400).json({ error: "Demasiadas líneas de una vez." });
    ...
    for (const l of entrantes) {
      ...
      if (!(emp.clientes || []).includes(cliId)) {
        return res.status(400).json({ error: "Ese cliente no está asignado a ti." });
      }
      if (!esFecha(fecha) || !esHora(entrada) || !esHora(salida)) {
        return res.status(400).json({ error: "Revisa el día y las horas." });
      }
      if (fecha > hoy.fecha) return res.status(400).json({ error: "No puedes apuntar días que aún no han llegado." });
      if (fecha < limite) return res.status(400).json({ error: "Solo puedes apuntar días de los últimos 2 meses." });
      if (minutosEntre(entrada, salida) === 0) {
        return res.status(400).json({ error: "La hora de entrada y la de salida no pueden ser la misma." });
      }
      preparadas.push({ cliId, fecha, entrada, salida, notas: txt(l.notas, 300) });
    }

    // No repetir lo que ya está apuntado (evita duplicados al pulsar dos veces)
    const fechas = [...new Set(preparadas.map((l) => l.fecha))];
    const yaHay = await registrosEntre(fechas.reduce((a, b) => (a < b ? a : b)),
    ...
    for (const l of preparadas) {
      ...
      creados.push(quitarPrecio(decorar(reg)));
    }
    return res.status(200).json({ ok: true, creados, duplicados });
  }
  ...
  if (accion === "emp-clave") {
    ...
    emp.salt = crypto.randomBytes(16).toString("hex");
...
async function zonaAdmin(accion, body, req, res) {
  ...
  if (accion === "admin-datos") {
    ...
    return res.status(200).json({
      empleadas: emps.map(({ salt, hash, ...e }) => e),
  ...
  if (accion === "emp-guardar" || accion === "admin-emp-guardar") {
    ...
    emp.telefono = txt(body.telefono, 30);
